...
The thesis would fill this gap by estimating leakage where it is most likely to appear: the North African corridor, whose four partners combine proximity, existing cement, fertiliser, and steel capacity, preferential access under Euro-Mediterranean agreements (European Commission, 2026), and no carbon pricing of their own within the sample period (World Bank, 2026). The question has direct policy stakes. The Carbon Border Adjustment Mechanism entered its definitive regime in January 2026, and its calibration presumes that carbon-cost differentials divert trade. A corridor-level estimate of the pre-CBAM baseline would inform this calibration and provide the empirical counterpart to simulation-based evaluations (Bellora and Fontagn\'{e}, 2023). The thesis would execute the research design developed in my seminar paper, which proposes the identification strategy but deliberately stops short of estimation.

\section{Methodology and Data (max.\ 250 words)}

The thesis would estimate a triple difference on EU import flows over 2000 to 2021, interacting product-level exposure $\tau_p$, the effective carbon cost $C_{st}$, and a lower-regulation corridor indicator $L_c$, with product-by-partner, partner-by-year, and product-by-year fixed effects. The comparison group ($L_c = 0$) is the Western Balkans, which priced no carbon. The two-way term $C_{st} \times L_c$ varies across sectors within a partner-year cell, so it is not absorbed and enters as a control.

The treatment is $C_{st} = P_t \times (1 - a_{st})$, where $P_t$ is the annual EUA price and $a_{st}$ is the sector-year free-allocation share measured against fixed 2005--2007 verified emissions and capped at one, since Phase~I and~II over-allocation would otherwise make the cost negative. Exposure $\tau_p$ is the CO$_2$ intensity of the supplying NACE Rev.~2 sector, fixed at 2008--2010, the earliest available.

Data are public: Eurostat Comext (DS-045409), the EU Transaction Log for allocation, requiring a NACE crosswalk, and Eurostat air emissions accounts, linked via the CN--CPA--NACE chain. Estimation would use Poisson pseudo-maximum likelihood, which corrects heteroskedasticity bias in log-linear models (Santos Silva and Tenreyro, 2006), alongside OLS with high-dimensional fixed effects.

The central assumption is parallel trends, tested by event study over the pre-announcement years 2000 to 2002. Four further challenges arise. Only four clusters are treated, so the wild bootstrap is unreliable and randomisation inference is needed. Regulated gas prices in Algeria and Egypt confound the estimate. The 2019 EU steel safeguard falls inside the strongest-treatment window. The pandemic years require an exclusion check.
...
Two features distinguish the proposed thesis from the closest existing study. Naegele and Zaklan (2019) never interact stringency with partner regulatory status, so an effect confined to one corridor is not tested, and their sample ends in 2011, before the price acceleration that supplies the strongest treatment. The proposed design separates partners and runs through 2021.
